...
class MaintenanceRequest(models.Model):
    ...
    maintenance_priority_id = fields.Many2one(
      ...
      string="Prioridad de mantenimiento",
      ondelete="restrict"
    )
    ...
    def create(self, vals):
      ...
      if 'maintenance_plan_id' in vals and vals['product_structure_ids']:
        res.product_structure_ids = [(5, 0, 0)]
        for rec in vals['product_structure_ids']:
          res.product_structure_ids = [(0,0, {
              'product_id': rec.product_id.id,
              'quantity': rec.quantity,
              'unit_of_measurement': rec.unit_of_measurement.id,
